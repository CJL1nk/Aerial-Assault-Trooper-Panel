...
int main(int argc, char *argv[])
{
    ...
    QWidget window;
    window.setWindowTitle("Aerial Assault Trooper DDOS Panel");
    ...
    // Run button
    QPushButton *startButton = new QPushButton("       Start DDOS       ");
    startButton->setStyleSheet("background: white; color: black;");
    startButton->setMaximumWidth(200);

    // Stop button
    QPushButton *stopButton = new QPushButton("       Stop DDOS       ");
    ...
    QObject::connect(startButton, &QPushButton::clicked, [&]() {
        qDebug("Starting super mega DDOS on: %s", qPrintable(ipInput->text()));
    });
